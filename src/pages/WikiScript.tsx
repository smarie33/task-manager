...
const WikiScript: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      const { data: scriptRows, error: scriptErr } = await supabase.from("wiki_scripts").select("id,name").eq("name", scriptName).limit(1);
      ...
      const { data: links, error: linkErr } = await supabase.from("wiki_entry_scripts").select("entry_id").eq("script_id", script.id);
